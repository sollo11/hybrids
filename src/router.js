...
function saveLayout(target) {
  focusMap.set(target, document.activeElement);
...
function restoreLayout(target, clear) {
  const activeElement = focusMap.get(target);
...
function setupView(name, view, parent) {
  ...
  if (!config) {
    ...
    callbacksMap.get(Constructor).push(drawLayout);
...
function router(views, options = {}) {
  ...
  return {
    get: (host, lastValue) => {
      if (lastValue && lastValue.length < stack.length) {
    ...
    observe(host, value, lastValue) {
      if (lastValue && value.length < lastValue.length) {
        restoreLayout(value[value.length - 1]);
      }
    },
  };
}
